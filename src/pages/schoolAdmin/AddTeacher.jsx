...
export default function AddTeacher() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (userData.id) {
        // 2. Create the teacher profile — only profile fields, no user fields
  ...
  return (
    <SchoolLayout title="Add Faculty">
      <div className="max-w-4xl space-y-6 px-4 md:px-8 py-4 md:py-6">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              {loading
                ? <><span className="material-symbols-outlined animate-spin text-[16px]">progress_activity</span> Registering...</>
                : <><span className="material-symbols-outlined text-[16px]">save</span> Add Faculty</>
              }
            </button>
          </div>
        </form>
