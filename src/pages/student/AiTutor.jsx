    ...
    <MainLayout title="Student Portal">
      <div className="p-4 md:p-6 h-[calc(100vh-80px)] flex flex-col gap-4 overflow-hidden">
        <div className="flex justify-between items-center flex-wrap gap-3">
          <div>
            <Skeleton className="w-48 md:w-56 h-7 md:h-8 mb-1" />
            <Skeleton className="w-56 md:w-72 h-4" />
          </div>
          <div className="flex gap-2">
            <Skeleton className="w-16 md:w-20 h-8 rounded-md" />
            <Skeleton className="w-24 md:w-28 h-8 rounded-md" />
          </div>
        </div>
...
export default function AiTutor() {
  ...
  return (
    <MainLayout title="Student Portal">
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-gray-900 text-white text-xs px-4 py-2 rounded-xl shadow-xl">
          <span>{toast.msg}</span>
          <button onClick={() => undoDelete(toast.undoId)} className="font-bold text-primary underline">Undo</button>
      ...
      <div className="p-4 md:p-6 h-[calc(100vh-80px)] flex flex-col gap-4 overflow-hidden">
        ...
        <div className="flex justify-between items-center flex-wrap gap-3">
          <div>
            <h2 className="text-xl md:text-2xl font-bold text-on-surface">AI Intelligent Tutor</h2>
            <p className="text-xs md:text-sm text-on-surface-variant">Your personal academic companion powered by ScholarFlow AI.</p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setShowTrash(true)}
              className="flex items-center gap-1 px-3 py-1.5 bg-surface-container-lowest text-primary text-sm font-medium rounded-md border hover:bg-surface-container-low">
              <span className="material-symbols-outlined text-base">delete</span>
              <span className="hidden sm:inline">Trash</span>
            </button>
            <button onClick={() => setShowNewModal(true)}
              className="flex items-center gap-1 px-3 py-1.5 bg-gradient-to-br from-primary to-primary-container text-white text-sm font-semibold rounded-md shadow-md">
        ...
        <div className="flex-1 flex gap-4 overflow-hidden">
          ...
          <div className={`fixed top-0 left-0 h-full w-72 bg-white z-50 transform transition-transform duration-300 ease-in-out lg:hidden shadow-xl ${
          ...
          <div className="hidden lg:block w-64 xl:w-72 h-full flex flex-col bg-white rounded-xl border shadow-sm overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto">
              {conversations.length === 0 ? (
                ...
                conversations.map(conv => (
                  <div key={conv.id} onClick={() => openConversation(conv.id)}
                    className={`px-3 py-2 cursor-pointer border-b hover:bg-gray-50 transition-colors ${activeConv?.id === conv.id ? 'bg-primary/5 border-l-2 border-l-primary' : ''}`}>
                    <p className="text-sm font-semibold truncate">{conv.title}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      {conv.subject && <span className="text-[9px] font-bold bg-primary/10 text-primary px-1.5 py-0.5 rounded-full">{conv.subject}</span>}
                      <span className="text-[9px] text-gray-400">{timeAgo(conv.updated_at)}</span>
            ...
            <div className="p-2 border-t">
              <button onClick={() => setShowTrash(true)} className="w-full flex items-center gap-2 px-2 py-1.5 text-xs text-gray-500 hover:bg-gray-100 rounded-lg">
                <span className="material-symbols-outlined text-sm">delete</span>
                Recently Deleted
              </button>
            </div>
          ...
          {/* Chat Window - Larger */}
          <div className="flex-1 h-full flex flex-col bg-white rounded-xl border shadow-sm overflow-hidden">
            {/* Chat Header - Compact */}
            <div className="px-4 py-2 border-b flex items-center justify-between bg-gray-50/30">
              <div className="flex items-center gap-2">
                <button onClick={() => setShowMobileDrawer(true)} className="lg:hidden p-1 rounded-lg hover:bg-gray-100">
                  <span className="material-symbols-outlined text-lg">menu</span>
                </button>
                {activeConv ? (
                  <div className="flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-green-500" />
                    <span className="text-sm font-semibold">{activeConv.title}</span>
                  ...
                  <span className="text-sm text-gray-400">Select a session to start</span>
                )}
              </div>
              <div className="flex items-center gap-2">
                ...
                (activeConv.messages || []).filter(m => m.role !== 'system').map(msg => (
                  msg.role === 'user' ? (
                    <div key={msg.id} className="flex justify-end">
                      <div className="max-w-[75%] bg-primary/10 text-on-surface p-3 rounded-2xl rounded-tr-none shadow-sm">
                        <p className="text-sm leading-relaxed whitespace-pre-wrap">{msg.content}</p>
                        <span className="text-[9px] mt-1 block opacity-50 text-right">{new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                    </div>
                  ) : (
                    <div key={msg.id} className="flex justify-start gap-2">
                      ...
                      <div className="max-w-[80%] bg-white border p-3 rounded-2xl rounded-tl-none shadow-sm">
                        <div className="text-sm leading-relaxed prose prose-sm max-w-none prose-headings:font-bold prose-p:my-1 prose-ul:my-1 prose-ol:my-1">
                          <ReactMarkdown remarkPlugins={[remarkMath]} rehypePlugins={[rehypeKatex]}>
                            {msg.content}
                          </ReactMarkdown>
                        </div>
                        <span className="text-[9px] mt-2 block opacity-40">AI Generated · {new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                    </div>
                  )
                ))
            ...
            {/* Retry Banner */}
            {retryBanner && (
              <div className="mx-3 mb-1 px-3 py-1.5 bg-red-50 border border-red-200 rounded-lg flex items-center justify-between">
                <p className="text-xs text-red-600">{typeof retryBanner === 'string' ? retryBanner : 'Something went wrong. Please try again.'}</p>
                <button onClick={() => setRetryBanner(false)} className="text-red-400 hover:text-red-600">
                  <span className="material-symbols-outlined text-sm">close</span>
                </button>
              </div>
            )}
            ...
            <div className="p-3 bg-white border-t">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 bg-gray-50 border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 disabled:opacity-50"
                />
                <button
                  onClick={sendMessage}
                  disabled={!activeConv || !inputMsg.trim() || isTyping}
                  className="w-8 h-8 bg-primary text-white rounded-xl flex items-center justify-center shadow-md hover:bg-primary/90 disabled:opacity-40"
                >
